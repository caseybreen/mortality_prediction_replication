...
Summary: Create a plot showing descriptive differences in longevity across population subgroups. 
...
## library packages and read in CenSoc-DMF  
...
## read in data 
dmf_linked <- read_csv(here("data", "censoc_dmf_v3_linked.csv")) %>%
  janitor::clean_names()
...
dmf_linked_1910 <- dmf_linked %>%
  ...
  ) %>% recode_education(educ_var = "educd") ## recode education g
```
...
## create final plot 
final_plot <- plot_grid(aggregate_plots)

## final plot 
ggsave(plot = aggregate_plots, filename = here("figures/aggregate_disparities_plot.png"), height = 5, width = 12)
```
...
## save plot 
ggsave(plot = heterogeneity_plot, filename  = here("figures", "03_heterogeneity_stochasticity.png"), width = 12, heigh = 8)
ggsave(plot = heterogeneity_plot, filename  = here("figures", "03_heterogeneity_stochasticity.svg"), width = 12, heigh = 8)
```
