...
calc_r2 <- function(actual, predicted) {
  ok <- stats::complete.cases(actual, predicted)
  actual <- actual[ok]; predicted <- predicted[ok]

  sst <- sum((actual - mean(actual))^2)
  ssr <- sum((actual - predicted)^2)   # <-- use 'predicted' here
  1 - ssr / sst
}
```
...
df %>% 
 ...
 # group_by(race_black_recode) %>% 
  summarize(r2 = calc_r2(superlearner, observed_death_age), cor(superlearner, observed_death_age))

calc_r2(dmf_holdout$death_age, df$superlearner)
...
df %>%
  bind_cols(dmf_holdout) %>%
    group_by(race_black_recode) %>% 
  summarize(
    r2  = calc_r2(observed_death_age, superlearner),
        mae  = calc_mae(observed_death_age, superlearner),
        rmse  = calc_rmse(observed_death_age, superlearner),
    cor = cor(superlearner, observed_death_age, use = "complete.obs")
  )

df %>%
  bind_cols(dmf_holdout) %>%
    group_by(educ_yrs > median(educ_yrs)) %>% 
  summarize(
    r2  = calc_r2(observed_death_age, superlearner),
    mae  = calc_mae(observed_death_age, superlearner),
        rmse  = calc_rmse(observed_death_age, superlearner),
...
df %>%
  bind_cols(dmf_holdout) %>%
    group_by(incwage_recode > median(incwage_recode)) %>% 
  summarize(
    r2  = calc_r2(observed_death_age, superlearner),
    mae  = calc_mae(observed_death_age, superlearner),
        rmse  = calc_rmse(observed_death_age, superlearner),
