---
title: "Prediction on CenSoc-DMF" 
author: Casey Breen
---

Summary: This notebook contains code to estimate the predictability of lifespan using the CenSoc-DMF data and machine learning models (specifically, an emsemble superlearner algorithm). On our training dataset, we train machine learning algorithms to predict age of death using CenSoc-DMF mortality records. On our test dataset, we assess the performance of the machine learning algorithms using popular model performance metrics (e.g., $R^2$). 
...
## save models 
ggsave(plot = combined_plot_dmf, filename = here("figures/04_combined_plot_dmf.png"), width = 14, height = 7)
ggsave(plot = combined_plot_dmf, filename = here("figures/04_combined_plot_dmf.svg"), width = 14, height = 7)
```
...
ggsave(plot = metrics_subgroup_plot, filename = here("figures", "appendix_metrics_subgroup_plot.png"), width = 9, height = 6, bg = "white")
```

## Variable importance from Permutation 

This takes a long time, so it is skipped by default. But it can be uncommented and ran. 
...
# 
# ## save plot
# ggsave(plot = variable_importance, filename = here("figures/appendix_variable_importance_permutation.png"), height = 8, width = 7)
```
