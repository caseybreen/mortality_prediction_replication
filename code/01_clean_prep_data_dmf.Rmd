---
title: "Prep CenSoc-DMF"
authors: Casey Breen and Nathan Seltzer
---

## Description

In this notebook, we prepare the CenSoc-DMF for our model. This involves reading in the full linked CenSoc-DMF file, restricting to the birth cohort of 1910, splitting it into a test/train set, and recoding variables.


```{r}
## source custom functions and library packages
library(here)
source("helpers.R")
...
dmf_linked_1910 <- dmf_linked %>%
  ...
  select(
    ...
    labforce, nfams, urban, farm, ownershpd, presgl, ncouples, nmothers,
...
```{r}
## recode education  
dmf_linked_1910 <- recode_education(dmf_linked_1910, educ_var = "educd")
...
## split data
split <- initial_split(dmf_linked_1910_recode, prop = 0.75) # set to .6
censoc_dmf_train_data <- rsample::training(split)
censoc_dmf_test_data <- rsample::testing(split)
```


```{r}
## save data
write_rds(x = censoc_dmf_train_data, file = here("data/censoc_dmf_train_data_new.rds"))
write_rds(x = censoc_dmf_test_data, file = here("data/censoc_dmf_test_data_new.rds"))
```
